...
class JogPanel(QScrollArea):
    ...
    def _build(self):
        ...
        self._nav_widgets   = [prev_btn, next_btn]
    ...
    def set_nav_enabled(self, enabled):
        """Enable or disable Previous/Next Point buttons independently."""
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        close_btn = _header_btn("Close File")
        close_btn.clicked.connect(self._close_file)
        self._close_file_btn = close_btn
    ...
    def _on_connection_changed(self, connected):
        ...
        # Nav buttons require BOTH a connection AND loaded points.
        self._set_nav_enabled(self._has_points())
    ...
    def apply_theme(self):
        ...
        """)
        # close file btn tinted muted/red
        m = T["muted"]
        self._close_file_btn.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {m};
                border: 1px solid {T['border']};
                border-radius: 6px; padding: 4px 12px; height: 30px;
            }}
            QPushButton:hover {{ background: rgba({_hex_to_rgb(T['red'])}, 0.12); color: {T['red']}; border-color: {T['red']}; }}
            QPushButton:pressed {{ background: rgba({_hex_to_rgb(T['red'])}, 0.25); }}
        """)
        # connection button
    ...
    def _set_nav_enabled(self, enabled):
        """Enable or disable Previous/Next Point buttons.

        Nav is only actually enabled when *both* conditions are true:
        points exist AND the robot is connected.  Call this whenever either
        condition changes.
        """
        can_nav = enabled and self._robot_connected
        self._jog_panel.set_nav_enabled(can_nav)

    def _has_points(self):
        return self._pos_table._table.rowCount() > 0

    def _reset_file_state(self):
        """Clear all file-related state without user interaction."""
    ...
    def _close_file(self):
        if self._pos_table.dirty:
            ans = QMessageBox.question(
                ...
                "You have unsaved changes. Discard and close the file?",
    ...
    def _open_file(self):
        ...
        try:
            ...
            self._reset_file_state()          # clear everything before loading
            self._pos_table.load_points(points)
            self._current_file = path
            self._file_label.setText(os.path.basename(path))
            self._set_nav_enabled(len(points) > 0)
    ...
    def _new_file(self):
        ...
        if not path:
            return
        self._reset_file_state()              # clear everything before starting fresh
        self._current_file = path
        self._file_label.setText(os.path.basename(path))
